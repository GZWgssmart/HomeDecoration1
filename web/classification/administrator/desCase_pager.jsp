    ...
    <div class="admin-content">
        <div class="admin-content-body">
            <div class="am-cf am-padding">
                <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg"><a href="javascript:history.go(-1)" >返回</a></strong> / <small>所有设计师案例</small></div>
            </div>
                    ...
                    <div class="am-panel am-panel-default">
                        <div class="am-panel-bd am-scrollable-horizontal">
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.desCaseList }" var="desCase" varStatus="status">
	                                <tr>
	                                    <td>${desCase.name }</td>
	                                    <td>${desCase.plot_name }</td>
	                                    <td>${desCase.style }</td>
	                                    <td>${desCase.des }</td>
	                                    <td>
	                                    	<c:if test="${desCase.status == 'Y' }">
		                                    	可用
		                                    </c:if>
		                                    <c:if test="${desCase.status == 'N' }">
		                                    	冻结
		                                    </c:if>
	                                    </td>
	                                    <td>${desCase.created_time }</td>
	                                    <td>
	                                        <a class="am-btn am-btn-primary" href="<%=path %>/BHdes/desCase?id=${desCase.id }">查看</a>
	                                        <c:if test="${desCase.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/desCasestop?id=${desCase.id }">停用</a>
	                                    	</c:if>
	                                    	<c:if test="${desCase.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/desCasepass?id=${desCase.id }">启用</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
							...
							<ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/desCasepager?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/admCon/desCasepager?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/desCasepager?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/admCon/desCasepager?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
