                        ...
                        <div class="am-panel-bd am-scrollable-horizontal">
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.comList }" var="com" varStatus="status">
	                                <tr>
	                                    ...
	                                    <td>${com.address }</td>
	                                    <td>
		                                    <c:if test="${com.status == 'Y' }">
		                                    	可用
		                                    </c:if>
		                                    <c:if test="${com.status == 'N' }">
		                                    	冻结
		                                    </c:if>
	                                    </td>
	                                    ...
	                                    <td>
	                                        ...
	                                        <c:if test="${com.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admMan/comstop?id=${com.id }">停用</a>
	                                    	</c:if>
	                                    	<c:if test="${com.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admMan/compass?id=${com.id }">启用</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
							...
							<ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admMan/compager?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/admMan/compager?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admMan/compager?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/admMan/compager?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
