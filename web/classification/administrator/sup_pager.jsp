                        ...
                        <div class="am-panel-bd am-scrollable-horizontal">
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.supList }" var="sup" varStatus="status">
	                                <tr>
	                                    ...
	                                    <td>${sup.address }</td>
	                                    <td>
	                                    	<c:if test="${sup.status == 'Y' }">
		                                    	可用
		                                    </c:if>
		                                    <c:if test="${sup.status == 'N' }">
		                                    	冻结
		                                    </c:if>
	                                    </td>
	                                    ...
	                                    <td>
	                                        ...
	                                        <c:if test="${sup.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admMan/supstop?id=${sup.id }">停用</a>
	                                    	</c:if>
	                                    	<c:if test="${sup.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admMan/suppass?id=${sup.id }">启用</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
							...
							<ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admMan/suppager?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/admMan/suppager?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admMan/suppager?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/admMan/suppager?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
