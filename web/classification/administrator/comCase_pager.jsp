    ...
    <div class="admin-content">
        <div class="admin-content-body">
            <div class="am-cf am-padding">
                <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg"><a href="javascript:history.go(-1)" >返回</a></strong> / <small>所有装修公司案例</small></div>
            </div>
                    ...
                    <div class="am-panel am-panel-default">
                        <div class="am-panel-bd am-scrollable-horizontal">
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.comCaseList }" var="comCase" varStatus="status">
	                                <tr>
	                                    <td>${comCase.name }</td>
	                                    <td>${comCase.plot_name }</td>
	                                    <td>${comCase.style }</td>
	                                    <td>${comCase.des }</td>
	                                    <td>
	                                    	<c:if test="${comCase.status == 'Y' }">
		                                    	可用
		                                    </c:if>
		                                    <c:if test="${comCase.status == 'N' }">
		                                    	冻结
		                                    </c:if>
	                                    </td>
	                                    <td>${comCase.created_time }</td>
	                                    <td>
	                                        <a class="am-btn am-btn-primary" href="<%=path %>/BHcompany/case?id=${comCase.id }">查看</a>
	                                        <c:if test="${comCase.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/comCasestop?id=${comCase.id }">停用</a>
	                                    	</c:if>
	                                    	<c:if test="${comCase.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/comCasepass?id=${comCase.id }">启用</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
							...
							<ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/comCasepager?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/admCon/comCasepager?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/comCasepager?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/admCon/comCasepager?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
