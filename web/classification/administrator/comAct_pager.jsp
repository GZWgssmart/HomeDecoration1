    ...
    <div class="admin-content">
        <div class="admin-content-body">
            <div class="am-cf am-padding">
                <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg"><a href="javascript:history.go(-1)" >返回</a></strong> / <small>所有装修公司活动</small></div>
            </div>
                    ...
                    <div class="am-panel am-panel-default">
                        <div class="am-panel-bd am-scrollable-horizontal">
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.comActList }" var="comAct" varStatus="status">
	                                <tr>
	                                    <td>${comAct.name }</td>
	                                    <td>${comAct.des }</td>
	                                    <td>
		                                    <c:if test="${comAct.status == 'Y' }">
		                                    	可用
		                                    </c:if>
		                                    <c:if test="${comAct.status == 'N' }">
		                                    	冻结
		                                    </c:if>
	                                    </td>
	                                    <td>${comAct.created_time }</td>
	                                    <td>
	                                        <a class="am-btn am-btn-primary" href="<%=path %>/BHcompany/active?id=${comAct.id }">查看</a>
	                                        <c:if test="${comAct.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/comActstop?id=${comAct.id }">停用</a>
	                                    	</c:if>
	                                    	<c:if test="${comAct.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/comActpass?id=${comAct.id }">启用</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
							...
							<ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/comActpager?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/admCon/comActpager?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/comActpager?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/admCon/comActpager?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
