                        ...
                        <div class="am-panel-bd am-scrollable-horizontal">
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.proList }" var="pro" varStatus="status">
	                                <tr>
	                                    ...
	                                    <td>
	                                        <a class="am-btn am-btn-primary" href="<%=path %>/BHsup/pro?id=${pro.id }">查看</a>
	                                        <c:if test="${pro.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/prostop?id=${pro.id }">下架</a>
	                                    	</c:if>
	                                    	<c:if test="${pro.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/admCon/propass?id=${pro.id }">上架</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
							...
							<ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/propager?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/admCon/propager?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/admCon/propager?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/admCon/propager?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
