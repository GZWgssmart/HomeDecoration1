    ...
    <div class="admin-content">
        <div class="admin-content-body">
            <div class="am-cf am-padding">
                <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg"><a href="javascript:history.go(-1)" >返回</a></strong> / <small>所有活动</small></div>
            </div>
                ...
                <div class="am-u-sm-12 am-u-lg-10 am-u-lg-offset-1">
                    <div class="am-panel am-panel-default">
                        <div class="am-panel-bd am-scrollable-horizontal">
                            <a class="am-btn am-btn-primary am-btn-block" href="<%=path %>/supAct/addpage">添加活动</a>
                            <hr>
                            ...
                            am-text-nowrap">
                                ...
                                <c:forEach items="${requestScope.actList }" var="act" varStatus="status">
	                                <tr>
	                                    <td>${act.name }</td>
	                                    <td>${act.des }</td>
	                                    <td>
	                                    	<c:if test="${act.status == 'Y' }">
		                                    	可用
		                                    </c:if>
		                                    <c:if test="${act.status == 'N' }">
		                                    	冻结
		                                    </c:if>
	                                    </td>
	                                    <td>${act.created_time }</td>
	                                    <td>
	                                        <a class="am-btn am-btn-primary" href="<%=path %>/supAct/show?id=${act.id }">查看</a>
	                                        <a class="am-btn am-btn-primary" href="<%=path %>/supAct/updatepage?id=${act.id }">修改</a>
	                                        <c:if test="${act.status == 'Y' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/supAct/actstop?id=${act.id }">下架</a>
	                                    	</c:if>
	                                    	<c:if test="${act.status == 'N' }">
	                                        	<a class="am-btn am-btn-primary" href="<%=path %>/supAct/actpass?id=${act.id }">启用</a>
	                                    	</c:if>
	                                    </td>
	                                </tr>
                                </c:forEach>
                            ...
                            <ul class="am-pagination am-pagination-centered">
                                <c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/supAct/acts?pageNo=1" class="am-btn">&laquo;首页</a></li>
	                                	<li><a href="<%=path %>/supAct/acts?pageNo=${requestScope.pageNo - 1 }" class="am-btn">上一页</a></li>
									</c:otherwise>
								</c:choose>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="<%=path %>/supAct/acts?pageNo=${requestScope.pageNo + 1 }" class="am-btn">下一页</a></li>
                                		<li><a href="<%=path %>/supAct/acts?pageNo=${requestScope.pageCount }" class="am-btn">尾页&raquo;</a></li>
									</c:otherwise>
								</c:choose>
